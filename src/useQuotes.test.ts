...
describe('sliceQuotes', () => {
  function makeQuotesMap(arr: [number, number][]) {
    // arr: [[price, size], ...]
    const map = new Map();
...
describe('quotesReducer', () => {
  it('should handle snapshot: set quotes and total size', () => {
    ...
    expect([...nextState.quotes.values()]).toEqual([
      { price: 100, isNewPrice: false, size: 10, sizeDelta: 0, currentTotalSize: 0 },
      { price: 101, isNewPrice: false, size: 20, sizeDelta: 0, currentTotalSize: 0 },
    ]);
  ...
  it('should handle delta updates: update existing quote size and size delta', () => {
    const nextState = quotesReducer(
      {
        quotes: new Map([
          [100, { price: 100, isNewPrice: false, size: 10, sizeDelta: 0, currentTotalSize: 0 }],
          [101, { price: 101, isNewPrice: false, size: 20, sizeDelta: 0, currentTotalSize: 0 }],
        ]),
        totalSize: 30,
      },
      {
    ...
    expect([...nextState.quotes.values()]).toEqual([
      { price: 100, isNewPrice: false, size: 15, sizeDelta: -5, currentTotalSize: 0 },
      { price: 102, isNewPrice: true, size: 5, sizeDelta: 0, currentTotalSize: 0 },
    ]);
